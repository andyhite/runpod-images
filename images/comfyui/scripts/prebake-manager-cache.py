...
CACHE_DIR = (
    Path(sys.argv[1])
    if len(sys.argv) > 1
    else Path("/opt/comfyui-baked/user/__manager/cache")
)
...
def fetch_registry_all() -> list:
    ...
    for page in range(2, total_pages + 1):
        ...
        if page % 20 == 0 or page == total_pages:
            print(
                f"  Registry: page {page}/{total_pages} ({len(all_nodes)} nodes total)"
            )
...
def main():
    ...
    try:
        ...
        print(
            f"  WARNING: Registry fetch failed ({e}), skipping — Manager will fetch on first start"
        )
